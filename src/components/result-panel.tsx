...
export default function ResultPanel({ data, isLoading, onShare }: ResultPanelProps) {
  ...
  const tldr = `Omnisence verdict on "${data.marketTitle}": ${data.verdict === "YES" ? "✅" : "❌"} ${data.verdict} (${data.confidence}% confidence). ${data.summary}`;
        ...
        <div className="flex items-start justify-between">
          <div className="space-y-3">
            ...
            <p className="text-lg text-neutral-700 dark:text-neutral-300">
              Omnisence thinks <span className="font-bold">{data.verdict}</span> based on {data.summary}
            </p>
          </div>
